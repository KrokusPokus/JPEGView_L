...
SET XHEIFSRC_DIR=%~dp0..\..\src\JPEGView\libheif
SET XAVIFSRC_DIR=%~dp0..\..\src\JPEGView\libavif
SET XLIBHEIF_DIR=%~dp0..\third_party\libheif
SET XLIBAVIF_DIR=%~dp0..\third_party\libavif
SET XOUT_DIR=%~dp0build_libheif_libavif
...
echo === HEADER FILES ===
echo Copying 'extras\third_party\libavif\include\avif\*.*' -to- 'src\JPEGView\libavif\include\avif\'
copy /y "%XLIBAVIF_DIR%\include\avif\*.h" "%XAVIFSRC_DIR%\include\avif\"
...
pushd "%XLIBHEIF_DIR%\dav1d"

REM or use --reconfigure flag
REM rd /s /q "%XLIBHEIF_DIR%\dav1d\build" 2>nul
...
REM copy /y "%XBUILD_DIR%\src\dav1d.dll" "%XHEIFSRC_DIR%\bin%~2\"
...
SET XLIBHEIF_DIR=%XLIBHEIF_DIR%\libde265
...
pushd "%XBUILD_DIR%"
cmake.exe -DCMAKE_BUILD_TYPE=Release -A %2 "%XLIBHEIF_DIR%"
IF ERRORLEVEL 1 exit /b 1
msbuild.exe /p:Platform=%2 /p:configuration="Release" libde265.sln /t:de265
IF ERRORLEVEL 1 exit /b 1


copy /y "%XBUILD_DIR%\libde265\Release\libde265.dll" "%XHEIFSRC_DIR%\bin%~3\"
IF ERRORLEVEL 1 exit /b 1
...
SET XDE265_DIR=%XLIBHEIF_DIR%\libde265
...
cmake.exe -DCMAKE_BUILD_TYPE=Release -A %2 -DDAV1D_LIBRARY="%XDAV1D_DIST%\lib\dav1d.lib" -DDAV1D_INCLUDE_DIR="%XDAV1D_DIST%\include" -DLIBDE265_LIBRARY="%XDE265_BUILD%\libde265\Release\de265.lib" -DLIBDE265_INCLUDE_DIR="%XDE265_DIR%" "%XLIBHEIF_DIR%\libheif"
IF ERRORLEVEL 1 exit /b 1
...
copy /y "%XBUILD_DIR%\libheif\Release\heif.dll" "%XHEIFSRC_DIR%\bin%~3\"
IF ERRORLEVEL 1 exit /b 1

copy /y "%XBUILD_DIR%\libheif\Release\heif.lib" "%XHEIFSRC_DIR%\lib%~3\"
IF ERRORLEVEL 1 exit /b 1

echo === HEADER FILES ===
echo Copying 'extras\third_party\libheif\libheif\libheif\*.*' -to- 'src\JPEGView\libheif\include\libheif\'
...
del /q "%XLIBHEIF_DIR%\libde265\libde265\de265-version.h"
...
SET XBUILD_DIR=%XLIBHEIF_DIR%\bin_%1
...
copy /y "%XBUILD_DIR%\libde265.dll" "%XHEIFSRC_DIR%\bin%~3\"
IF ERRORLEVEL 1 exit /b 1
...
REM for static dav1d:  cmake.exe -G"NMake Makefiles" -DCMAKE_BUILD_TYPE=Release -DDAV1D_LIBRARY="%XDAV1D_DIST%\lib\libdav1d.a" -DDAV1D_INCLUDE_DIR="%XDAV1D_DIST%\include" -DLIBDE265_LIBRARY="%XDE265_DIR%\bin_%XARCH%\lib\libde265.lib" -DLIBDE265_INCLUDE_DIR="%XDE265_DIR%" "%XLIBHEIF_DIR%\libheif"
cmake.exe -G"NMake Makefiles" -DCMAKE_BUILD_TYPE=Release -DDAV1D_LIBRARY="%XDAV1D_DIST%\lib\dav1d.lib" -DDAV1D_INCLUDE_DIR="%XDAV1D_DIST%\include" -DLIBDE265_LIBRARY="%XDE265_DIR%\bin_%XARCH%\lib\libde265.lib" -DLIBDE265_INCLUDE_DIR="%XDE265_DIR%" "%XLIBHEIF_DIR%\libheif"
IF ERRORLEVEL 1 exit /b 1
nmake.exe
IF ERRORLEVEL 1 exit /b 1
...
copy /y "%XBUILD_DIR%\libheif\heif.dll" "%XHEIFSRC_DIR%\bin%~3\"
IF ERRORLEVEL 1 exit /b 1
copy /y "%XBUILD_DIR%\libheif\heif.lib" "%XHEIFSRC_DIR%\lib%~3\"
IF ERRORLEVEL 1 exit /b 1
